"use client";

import { use } from "react";
import Link from "next/link";
import { AppShell } from "@/components/layout/AppShell";
import { AgentsPanel } from "@/components/agents/AgentsPanel";

// Workspace AI agents page — manage the AI team: view agents, add custom ones,
// edit their profiles, activate/deactivate, delete, and restore the defaults.
// All access and mutation rules are enforced by the API; this page renders
// controls based on the current user's role.

export default function AgentsPage({
  params,
}: {
  params: Promise<{ workspaceId: string }>;
}) {
  const { workspaceId } = use(params);

  return (
    <AppShell subtitle="AI agents">
      <div className="mb-6 flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">AI team</h1>
          <p className="mt-1 text-sm text-neutral-500">
            Your AI teammates. Each has a stable identity separate from its
            underlying provider and model.
          </p>
        </div>
        <Link
          href={`/workspace/${workspaceId}`}
          className="shrink-0 text-sm text-neutral-400 underline hover:text-neutral-200"
        >
          Back to workspace
        </Link>
      </div>

      <AgentsPanel workspaceId={workspaceId} />
    </AppShell>
  );
}
